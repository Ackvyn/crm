 ...
 * Signing is visual e-sign (drawn/typed PNG ink) in the browser, then the
 * Worker adds a document-level PKCS#7 self-signed seal and then a trusted
 * RFC 3161 completion timestamp so Acrobat/Edge show cryptographic proof.
 * The site identity remains self-signed, not a public-CA / QES certificate.
 * Number fields are CRM tool-level (digits-only) and bake as plain text.
...
<main>
  <h1>Review &amp; sign</h1>
  <p class="sub">Electronic signature with audit trail — visual ink, a self-signed site integrity seal, and a trusted RFC 3161 completion timestamp. Not a government-certified / qualified electronic signature (QES).</p>
...
async function submitSign() {
  ...
  try {
    ...
    // NEW CODE - TESTING: session PDF is pre-flattened, but if the library
    // source was overwritten with download-style /Sig widgets and any survived
    // bake, strip them so the visual e-sign ink is the only signature layer.
    try {
      const form = pdfDoc.getForm();
      const existing = form.getFields();
      for (let i = existing.length - 1; i >= 0; i--) {
        try {
          form.removeField(existing[i]);
        } catch (_) {
          /* stubborn widget */
        }
      }
    ...
    for (const field of envelope.fields || []) {
      ...
      if (field.type === "signature" || field.type === "initials") {
        ...
        if (cap && cap.png) {
          ...
          // Opaque base under ink (matches prepared share PDF field bases).
          page.drawRectangle({
