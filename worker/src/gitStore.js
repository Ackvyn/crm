...
export async function appendSignedPdfToTicket(env, siteKey, input) {
  ...
  const auditLines = [
    ...
    // NEW CODE - TESTING: PKCS#7 site seal metadata
    input.cryptoSealed
      ? "Crypto seal: PKCS#7 self-signed site certificate (Acrobat digital signature panel)"
      : null,
    input.sealFingerprint
      ? `Seal fingerprint (SHA-256): ${input.sealFingerprint}`
      : null,
    input.trustedTimestamped
      ? `Trusted timestamp: ${input.trustedTimestamp?.genTime || "RFC 3161"}`
      : null,
    input.trustedTimestamp?.tsaUrl
      ? `Timestamp authority: ${input.trustedTimestamp.tsaUrl}`
      : null,
    input.trustedTimestamp?.serialNumber
      ? `Timestamp serial: ${input.trustedTimestamp.serialNumber}`
      : null,
    input.certUrl ? `Site cert: ${input.certUrl}` : null,
    input.verifyUrl ? `Verify PDF: POST pdfBase64 → ${input.verifyUrl}` : null,
  ]
    .filter(Boolean)
    .join("\n");
