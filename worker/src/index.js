...
/** Public signer UI + complete + cert/verify — NOT agent `sign/sessions`. */
function isPublicSignPath(pathname) {
  ...
  // NEW CODE - TESTING: public site seal cert + verify
  if (/^\/v1\/[^/]+\/sign\/(cert|verify)\/?$/.test(pathname)) return true;
