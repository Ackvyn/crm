...
import {
  ...
  sha256HexOfPdf,
  verifySealedPdf,
  verifyTrustedTimestamps,
} from "./pdfCryptoSign.js";
import {
...
export class CrmSite {
  ...
  async fetch(request) {
    ...
    // NEW CODE - TESTING: share-link visual e-sign
    ...
    // NEW CODE - TESTING: public site seal cert + PDF verify (before token catch-all)
    if (path === "sign/cert" && request.method === "GET") {
      return this.handleSignCert(request);
    }
    if (path === "sign/verify" && request.method === "POST") {
      return this.handleSignVerify(request);
    }
    m = path.match(/^sign\/([^/]+)$/);
  ...
  async handleSignComplete(token, request) {
    ...
    const signerName = String(body.signerName || row.contact_name || "")
      .trim()
      .slice(0, 120);
    const signedSha256Client = String(body.signedSha256 || "").slice(0, 128);
    const ts = now();
    ...
    // NEW CODE - TESTING: visual ink arrives from the browser; Worker adds a
    // document-level PKCS#7 seal with this site's self-signed identity so
    // Acrobat/Edge show a real digital signature (not CA-trusted / not QES).
    let attachB64 = signedPdfBase64;
    let signedSha256 = signedSha256Client;
    let cryptoSealed = false;
    let sealFingerprint = null;
    let sealError = null;
    let trustedTimestamped = false;
    let trustedTimestamp = null;
    let timestampError = null;
    try {
      const visualBytes = base64ToBytes(signedPdfBase64);
      const identity = ensureSigningIdentity(
        this.ctx.storage.sql,
        this.ctx.id.name,
      );
      const sealed = await cryptoSealPdf(visualBytes, identity, {
        name: signerName || row.contact_name || "Signer",
        contactInfo: row.contact_email || "",
        location: `Ackvyn CRM · ${this.ctx.id.name}`,
        reason: `Electronically signed by ${signerName || row.contact_name || "signer"} via Ackvyn CRM share link`,
      });
      let finalBytes = sealed;
      cryptoSealed = true;
      sealFingerprint = identity.fingerprint;
      try {
        const timestamped = await addTrustedTimestamp(sealed, {
          siteKey: this.ctx.id.name,
          contactInfo: row.contact_email || "https://ackvyn.org",
        });
        finalBytes = timestamped.pdf;
        trustedTimestamped = true;
        trustedTimestamp = {
          genTime: timestamped.genTime,
          policy: timestamped.policy,
          serialNumber: timestamped.serialNumber,
          hashAlgorithm: timestamped.hashAlgorithm,
          tsaUrl: timestamped.tsaUrl,
        };
      } catch (err) {
        timestampError =
          err instanceof Error ? err.message : "timestamp_failed";
        // Keep the valid site seal when the public TSA is temporarily down.
      }
      attachB64 = bytesToBase64(finalBytes);
      signedSha256 = await sha256HexOfPdf(finalBytes);
    } catch (err) {
      sealError = err instanceof Error ? err.message : "seal_failed";
      // Fall back to visual-only PDF — signing must still succeed.
    }

    const origin = new URL(request.url).origin;
    const site = this.ctx.id.name;
    const verifyUrl = `${origin}/v1/${site}/sign/verify`;
    const certUrl = `${origin}/v1/${site}/sign/cert`;

    try {
      await appendSignedPdfToTicket(this.env, this.ctx.id.name, {
        ticketId: row.ticket_id,
        filename: `${baseName}-signed.pdf`,
        dataBase64: attachB64,
        contactEmail: row.contact_email,
  ...
  /** Public PEM for this site's self-signed e-sign seal. */
  handleSignCert(request) {
    const identity = ensureSigningIdentity(
      this.ctx.storage.sql,
      this.ctx.id.name,
    );
    ...
    return json({
      ...
      certificatePem: identity.certPem,
      verifyUrl: `${origin}/v1/${site}/sign/verify`,
      caveat:
        "Self-signed site seal — not a public CA / not a qualified electronic signature (QES). Use /sign/verify to check integrity against this certificate.",
    });
  }

  /** POST { pdfBase64 } → integrity + site-cert match. */
  async handleSignVerify(request) {
    let body;
    try {
      body = await request.json();
    } catch {
    ...
    const pdfBase64 = String(body.pdfBase64 || body.signedPdfBase64 || "").replace(
      /\s+/g,
      "",
    );
    if (!pdfBase64) return json({ error: "pdf_required" }, 400);
    const approx = Math.floor((pdfBase64.length * 3) / 4);
    if (approx > Math.floor(4 * 1024 * 1024)) {
    ...
    const siteSeal = verifySealedPdf(pdfBytes, identity.certPem);
    let trustedTimestamp;
    try {
      trustedTimestamp = await verifyTrustedTimestamps(pdfBytes);
    } catch (err) {
      trustedTimestamp = {
        ok: false,
        count: 0,
        timestamps: [],
        error:
          err instanceof Error ? err.message : "timestamp_verify_failed",
      };
    }
    return json({
      ok: Boolean(siteSeal.ok),
      fullyVerified: Boolean(siteSeal.ok && trustedTimestamp.ok),
      siteSeal,
      trustedTimestamp,
      site: this.ctx.id.name,
      siteFingerprintSha256: identity.fingerprint,
      siteCommonName: identity.commonName,
    });
  }
}
